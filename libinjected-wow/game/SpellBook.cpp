...
void SpellBook::castSpell(const WowGame& game, const uint32_t spellId, const uint128_t* target) {
	...
	(game.getFunction<SpellBook_CastSpell>(WowGameOffsets::WowGame::FunctionSpellBook_CastSpell))(spellIdx, 0, target, 0, 0);
}
...
typedef void(__fastcall PetInfo_SendPetAction)(uint64_t* spell, const WowGuid128* target, uint32_t unknown, uint64_t* unkwown2);
void SpellBook::petInfoSendPetAction(const WowGame& game, uint64_t* spell, const WowGuid128* target, uint32_t unknown, uint64_t* unkwown2) {
